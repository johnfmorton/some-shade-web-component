...
The component works in any framework. Import `some-shade` once to register the custom element, then use `<some-shade-image>` in your templates.
...
```ts
import { register } from 'some-shade';
...
```ts
import { register, get, list, SomeShadeImage } from 'some-shade';
